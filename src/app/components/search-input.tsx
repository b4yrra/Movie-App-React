  ...
  return (
    <div className="relative">
...

      {
        <div>
          {movies.length > 0 && (
            <div className="flex flex-col absolute left-0 top-full mt-2 z-50 bg-white dark:bg-[#0b0b0b] rounded-lg shadow-lg p-3 gap-3">
              {movies.slice(0, 5).map((movie) => (
                <Link
                  href={`/${movie.id}`}
                  key={movie.id}
                  className="flex rounded-lg cursor-pointer transition-transform duration-300 ease-in-out group brightness-80"
                >
                  <div className="flex items-center gap-5 rounded-lg bg-[#F4F4F5] dark:bg-[#27272A] max-w-full w-134 h-29 p-8">
                    <img
